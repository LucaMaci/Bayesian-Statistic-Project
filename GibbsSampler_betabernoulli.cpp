...
Rcpp::List GibbsSampler_betabernoulli( double alpha, double theta, double sigma_x,double sigma_a,  int n_tilde,  int n,  SEXP A_, SEXP X_, unsigned n_iter, unsigned initial_iters){
  ...
  */
  ...
  //  std::cout<< Z << std::endl;
    ...
    //  Rcpp::Rcout << eq_21_log_denominator << " + "<< eq_21_log_exponential << " = " << eq_21_log <<std::endl;
         
         
        // eq 12 dopo averla messa nel logaritmo
         
        long double eq_12_log_fraction = log(compute_cardinality(Z));
        Rcpp::Rcout << eq_12_log_fraction << std::endl;
        ...
        long double eq_12_log = eq_12_log_fraction + eq_12_log_product; 
